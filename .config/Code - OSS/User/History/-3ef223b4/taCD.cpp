        ...
        if (isErasing) {
            ...
            switch(selectedBrush) {
                ...
                case 2:
                    ...
                    break;
                case 3:
                    watercolourBrush(previousMousePos.x, previousMousePos.y, brushSize, gestureMagnitude, gestureDirection, red, green, blue);
                    break;
                case 4:
                    imageBrush(mouseX, mouseY, brushSize);
                    break;
                case 5:
                    funBrush(mouseX, mouseY, gestureMagnitude, gestureDirection);
                    break;
            }
        }
    }
...
// draws a circle using red green and blue variables
// doubles up as the eraser brush
//--------------------------------------------------------------
...
// draws multiple ellipses to create a watercolour type effect (with random opacity)
//
//--------------------------------------------------------------
void ofApp::watercolourBrush(float x, float y, int size, float speed, vec2 direction, int r, int g, int b){
    float scale = ofMap(speed, 1, 50, 0.1, 3, true) * size / 50;
    float angle = atan2(gestureDirection.y, gestureDirection.x);
    vec2 limited = direction * ofClamp(speed, 1, 30);
    ...
    ofPushMatrix();
        ...
        ofRotateRad(angle);
        for (int i = 0; i < 4; i++) {
            short offset = sin(ofRandom(100)) * 50;
            r = r + offset;
            g = g + offset;
            b = b + offset;
            ofSetColor(r, g, b, 20 + ofRandom(80));
            ofDrawEllipse(limited.x - sin(ofRandom(100)) * scale, limited.y + cos(ofRandom(100)) * scale, 5 * scale, 5 * scale);
        }
    ofPopMatrix();
}

// draws an image centred on mouse coordinates
//
//--------------------------------------------------------------
void ofApp::imageBrush(float x, float y, int size){
    ofLogo.draw(x - size / 2, y - size / 2, size, size);
}

// from kristiana pizica on the forum
// draws cool spikes !
//--------------------------------------------------------------
